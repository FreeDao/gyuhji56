...
public class WordHashLink {
	...
	static int checkConcat(String fromTokens[], String[]toTokens, int MinMatch){
		...
		int ret = -1;
		for(int x = 0; x < fromTokens.length - MinMatch; x ++){
...
//				Consoler.readString("~");
	...
	public void concat() throws IOException{
		LineReader lr = new LineReader("/ulks/sims-bak.txt");
		lw = new LineWriter("/ulks/sims-concat.txt", false);
		while(lr.hasNext()){
			if(lr.currentLineNumber() % 10 == 0){
			...
			for(int x = 1; x < tokens.length; x ++){
				int pos = tokens[x].indexOf('(');
				int num = Integer.parseInt(tokens[x].substring(0, pos));
				to[x - 1] = num;
			}
			ana(fromIndex, to);
		}
	...
	public static void main(String[]args) throws IOException{
		WordHashLink h = new WordHashLink();
		h.loadLines();
		h.concat();
	}
}
