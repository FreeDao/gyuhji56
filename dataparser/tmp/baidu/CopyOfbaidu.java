...
public class CopyOfbaidu extends Thread{
	int tNum;
	CopyOfbaidu(int tNum){
		this.tNum = tNum;
		log = new LogHelper("t-" + tNum);
	}
	static final File base = new File("e:/baidu/");
...
//				url = new URL("http://www.sogou.com/web?query=" + URLEncoder.encode(x, "utf-8"));
	...
	public static void main(String[] args) throws IOException {
		LineReader lr = new LineReader(
				new File("e:/行业分类.txt"), Charsets.GBK_CS);
		while(lr.hasNext()){
			String w = lr.next();
			stack.push(w);
//			System.out.println(w);
		...
		for(int x = 0; x < thread; x ++){
			CopyOfbaidu b = new CopyOfbaidu(x);
			b.start();
		}
	}
}
