...
import com.bmtech.htmls.parser.NodeFactory;
...
public class Lexer implements Serializable,NodeFactory{
	...
	/***
	 * changed by mudie at Jan.16.2007
	 * this function is designed for ScriptTag
	 * becouse Script tag usually contain some contains which 
	 * like tag .when using nestNode(boolean) ,sometimes error
	 * may occur.
	 * @return
	 * @throws ParserException
	 */
	public Node nextNode4Script() throws ParserException{

		char ch;
		Cursor cs=mCursor.dup();
		while(true){
			ch = mPage.getCharacter (cs);
			if (Page.EOF == ch){
			...
			//			cs.setPosition(cs.getPosition()-9);
			this.mCursor=cs.dup();

			Vector<PageAttribute> v=new Vector<PageAttribute>();
			PageAttribute p=new PageAttribute();
			p.setName("/script");
			v.add(p);
			return makeTag(cs.getPosition(),cs.mPosition+9,v);
		}
	 ...
	 */
	public Node makeString (int start, int end)
	throws
	ParserException
	{
		int length;
	 ...
	 */
	private void whitespace (Vector<PageAttribute> attributes, int[] bookmarks)
	{
	 ...
	 * @param bookmarks The array of positions.
	 */
	private void standalone (Vector<PageAttribute> attributes, int[] bookmarks)
	{
		attributes.addElement (new PageAttribute (
	 ...
	 * @param bookmarks The array of positions.
	 */
	private void empty (Vector<PageAttribute> attributes, int[] bookmarks)
	{
		attributes.addElement (new PageAttribute (
	 ...
	 * @param bookmarks The array of positions.
	 */
	private void naked (Vector<PageAttribute> attributes, int[] bookmarks)
	{
		attributes.addElement (new PageAttribute (
	 ...
	 * @param bookmarks The array of positions.
	 */
	private void single_quote (Vector<PageAttribute> attributes, int[] bookmarks)
	{
		attributes.addElement (new PageAttribute (
	 ...
	 * @param bookmarks The array of positions.
	 */
	private void double_quote (Vector<PageAttribute> attributes, int[] bookmarks)
	{
		attributes.addElement (new PageAttribute (
...
//	}
	...
	ParserException
	{
		...
		char ch;
		int state;
		Vector<PageAttribute> attributes;
		int code;

		done = false;
		state = 0;
		code = 0;
		attributes = new Vector<PageAttribute> ();
		// <%xyz%>
		// 012223d
		// <%=xyz%>
		// 0122223d
