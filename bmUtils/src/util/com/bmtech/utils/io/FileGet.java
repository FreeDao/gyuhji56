...
public class FileGet {
	...
	private final String retString = null;
	...
	public FileGet(URL url, int connectTimeout, int readTimeout, String charset)
			throws IOException {
	...
	public FileGet(URL url, int connectTimeout, int readTimeout)
			throws IOException {
	...
	public FileGet(String str) throws IOException {
		if (((str.length() >= 7) && (str.substring(0, 7).compareToIgnoreCase(
				"http://") == 0))
				|| ((str.length() >= 8) && (str.substring(0, 8)
						.compareToIgnoreCase("https://") == 0))) {
			try {
				URL url = new URL(str);
				this.url = url;
				urlGet();
	 ...
	 * @throws IOException
	 */
	public int getBin(String s) throws IOException {
		if (s == null)
			return -1;
		File f = new File(s);
		return getBin(f);
	 ...
	 * @throws IOException
	 */
	public int getBin(File file) throws IOException {
		if (bis == null)
			return -1;
	...
	private void urlGet() throws IOException {
		...
		if (proxy != null)
			...
			urlcnt = (HttpURLConnection) url.openConnection();
		/**
		 * 
		 * Accept: * /* Referer: http://www.133.net/ Accept-Language: zh-cn
		 * User-Agent: Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1;
		 * Trident/4.0; .NET CLR 2.0.50727; .NET CLR 3.0.04506.648; .NET CLR
		 * 3.5.21022; CIBA) Accept-Encoding: gzip, deflate
		 * 
		 */
		urlcnt.addRequestProperty("Accept", "*/*");
		urlcnt.addRequestProperty("Referer", "http://www.baidu.com");
		urlcnt.setRequestProperty("User-Agent",
	...
	public static Proxy setProxy(String host, int port)
			throws UnknownHostException {
		return proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(
	...
	public static String getStr(File file) throws IOException {
		return new FileGet(file).get(null);
	}

	public static String getStr(File file, Charset cs) throws IOException {
		return new FileGet(file).get(cs);
	}
	...
	public static void saveTo(File from, File to) throws IOException {
