...
public class IteratorableCrawler {
	...
	protected AtomicInteger errorNum = new AtomicInteger(0),
			crawledNum = new AtomicInteger(0), skipNum = new AtomicInteger(0);
	...
	public IteratorableCrawler(MDir mdirx, UrlIterator gen,
			int connectTimeoutMs, int readTimeoutMs) throws IOException {
		this.connectTimeoutMs = connectTimeoutMs;
		this.readTimeoutMs = readTimeoutMs;
		this.mdir = mdirx;
		this.genor = gen;
		final LogHelper log = new LogHelper("reporter");
					...
					try {
						...
						log
								.fatal(
										...
										genor.readedLine(), crawledNum,
										skipNum, errorNum, mdir.size());
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
				}
	...
	public class CrawlThread extends Thread {
		...
		CrawlThread(int id) {
			...
			hdl
					.setAgent("Mozilla/5.0 (Windows NT 5.1; rv:31.0) Gecko/20100101 Firefox/31.0");
		...
		public void crawl(URL url, MFile mfile, MFileWriter writer)
				throws IOException {
			lastUrl = url.toString();
			HttpCrawler crl = new HttpCrawler(url, hdl);
			try {
				crl.connect();
				MFileOutputStream ops;
				if (HttpCrawler.Gzip_Encoding == crl.getContentType()) {
					ops = writer.openMFile(mfile);
					crl.rawDataDumpTo(ops);
				} else {
					ops = writer.openMFileGzip(mfile);
					crl.dumpTo(ops);
				}
				ops.close();
			} finally {
			...
			log.warn("crawl %s, size %s KB, from url %s", mfile, mfile
					.getLength() / 100 / 10.0, url);
		}
	...
	public static IteratorableCrawler crawl(MDir mdirx, UrlIterator itr,
			int threadsNumber, int crawlInterval, int connectTimeoutMs,
