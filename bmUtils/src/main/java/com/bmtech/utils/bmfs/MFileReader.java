...
public class MFileReader {
	...
	MFileReader(MFile mfile, RandomAccessFile rafs) throws IOException {
		...
		if (fsId != this.mfile.fsId) {
			throw new IOException(
					String.format(
							"Strange! fsid not match! expect %s, but get %s for mfile %s",
		...
		public int read(byte[] buf, int offset, int len) throws IOException {
			...
			if (ret == -1) {
				throw new MFileFormatErrorException(
						"premature stream! should left " + left + ", require "
								+ len);
			}
			readed += ret;
			return ret;
	...
	private ReadAllInputStream getInputStream(boolean unGzip)
			throws IOException {
