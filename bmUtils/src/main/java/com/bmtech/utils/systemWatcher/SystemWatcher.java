...
public class SystemWatcher {
	...
	public SystemWatcher(int port, String key, String sysName) throws IOException {
		...
		this.regAction(new ShutDownAction(key));
		this.regAction(new SysInfoAction(key, sysName));
		this.regAction(new GCAction(key)); 
		this.regAction(new EchoAction(key));
		this.regAction(new SysPropAction(key));
		this.regAction(new WatcherAction(WatcherAction.HELP, key) {
			@Override
		...
		this.regAction(new WatcherAction("axp", key) {
			@Override
			public void run(String[] paras, Socket clientSocket)
			throws Exception {
