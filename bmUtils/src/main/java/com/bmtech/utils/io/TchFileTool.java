...
/**
 * this class define a model to serialize data: using a file to keep a special
 * token, only read/write this token
 * 
 * @author beiming
 * 
 */
public class TchFileTool {
	...
	public String read(String id) {
		...
		if (file.exists()) {
			try {
				LineReader lr = new LineReader(file, "utf-8");
				String ret = lr.readLine();
				lr.close();
				return ret;
			} catch (Exception e) {
				BmtLogger.instance().log(e,
						"can not read a touch action, id = %s", id);
				return null;
			}
		} else {
			return null;
	 ...
	 * 
	 * @param id
	 * @return the number readed, if nothing read or format is not a number,
	 *         return 0
	 */
	public int readInt(String id, int dft) {
		...
		if (str == null) {
			...
			try {
				...
				BmtLogger.instance().log(e,
						"can not parse a touched number, id = %s, token = %s",
						id, str);
				return dft;
			}
		}
	...
	public boolean write(String id, String token) {
		...
		try {
			...
			LineWriter lw = new LineWriter(file, false, Charset
					.forName("utf-8"));
			lw.writeLine(token);
			lw.close();
			return true;
			...
			BmtLogger.instance().log(e,
					"can not make a touch action, id = %s, token = %s", id,
					token);
			return false;
		}
	}
	...
	public static boolean multyPut(String path, String key, String[] args) {
		TchFileTool tft = new TchFileTool(path);
		boolean ret = true;
	...
	public static String[] multiGet(String path, String key, int numToRead) {
