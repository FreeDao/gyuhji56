...
import com.bmtech.utils.rds.SourceDefine.DataSourceDefine;
import com.bmtech.utils.rds.SourceDefine.Sqls;
import com.bmtech.utils.rds.convert.ResultSetConvert;
...
public class RDS {
	...
	private RDS(String source, Connection conn) {
		...
		if (this.conn == null) {
			...
			BmtLogger.instance().log(LogLevel.Debug,
					"make private connection for rds %s", source);
		} else {
			shared = true;
		}
	}
	...
	/**
	 * 
	 * @return before not access and using private conntion, return null; after
	 *         accessing, return the connection this instance used
	 */
	 ...
	 * @throws SQLException
	 */
	private void access(String sql, boolean largeResultSetSupport)
			throws SQLException {
		if (sql == null)
			throw new SQLException("sql '" + sql + "' is not defined");
		this.sql = sql;
		DataSourceDefine def = SourceDefine.instance().getDataSourceDefine(
				source);
		if (conn == null) {
			if (def == null)
				throw new SQLException(
						"sourceDefine '"
								+ source
								+ "' is not defined, besure defined at SRC block and detailed descript");
	...
	/**
	 * close connection and other database source. <br>
	 * iif the connection is shared, this operation will not close the
	 * connection,
	 */
	public void close() {
		if (pps != null) {
			try {
				...
				try {
					...
					BmtLogger.instance().log(e1,
							"got exception when colse pps", e);
				}
			}
			pps = null;
		}
		if (!shared) {
			if (conn != null) {
				BmtLogger.instance().log(LogLevel.Debug, "close connection");
				try {
					if (!conn.isClosed()) {
						conn.close();
					}
					...
					try {
						...
						BmtLogger.instance().log(e1,
								"got exception when colse conn", e);
					}
				}
				conn = null;
			}
	...
	public <T> List<T> load(Class<? extends T> clazz) throws Exception {
		ResultSetConvert<T> cvt = new ResultSetConvert<T>(clazz);
		return load(cvt);
	}

	public <T> List<T> load(ResultSetConvert<T> cvt) throws Exception {
		ResultSet rs = this.load();
		List<T> list = cvt.getListFromResult(rs);
		return list;
	}
	...
	public static byte[] getBlob(ResultSet rs, int idx) throws SQLException,
			IOException {
	 ...
	 * @throws SQLException
	 */
	public static RDS getRDSByKey(String key) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public static RDS getRDSByKey(String key, boolean largeResultSetSupport)
			throws SQLException {
		return getRDSByKey(key, largeResultSetSupport, null);
	}

	public static RDS getRDSByKey(String key, Connection conn)
			throws SQLException {
	...
	public static RDS getRDSByDefine(String source, String sql, Connection conn)
			throws SQLException {
	...
	static class SqlSplit {
		...
		public SqlSplit(String sql) throws SQLException {
			...
			if (str.length != 2) {
				throw new SQLException("'" + sql
						+ "' is not splitable or format error");
			}
			source = str[0];
			this.sql = str[1];
		}
		 ...
		 * @throws SQLException
		 */
		RDS getAndAccessRDS(boolean largeResultSetSupport, Connection conn)
				throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public static RDS getRDSByDefine(String sql, boolean largeResultSetSupport)
			throws SQLException {
		return getRDSByDefine(sql, largeResultSetSupport, null);
	}

	public static RDS getRDSByDefine(String sql, Connection conn)
			throws SQLException {
