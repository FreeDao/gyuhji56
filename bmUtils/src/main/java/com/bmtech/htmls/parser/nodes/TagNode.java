...
//
// $Source: /liying/bmt_base/src/htmlps/com/bmtech/htmls/parser/nodes/TagNode.java,v $
// $Author: liying $
...
package com.bmtech.htmls.parser.nodes;
...
import com.bmtech.htmls.parser.lexer.Cursor;
import com.bmtech.htmls.parser.lexer.Lexer;
import com.bmtech.htmls.parser.lexer.Page;
import com.bmtech.htmls.parser.scanners.Scanner;
import com.bmtech.htmls.parser.scanners.TagScanner;
import com.bmtech.htmls.parser.util.ParserException;
import com.bmtech.htmls.parser.visitors.NodeVisitor;

/**
 * TagNode represents a generic tag. If no scanner is registered for a given tag
...
public class TagNode extends AbstractNode implements Tag {
	 ...
	 */
	protected static Hashtable<String, Boolean> breakTags;
	static {
	...
	public TagNode(TagNode tag, TagScanner scanner) {
		this(tag.getPage(), tag.getTagBegin(), tag.getTagEnd(), tag
				.getAttributesEx());
	...
	public void setAttribute(Attribute attribute) {
		...
		if (!replaced) {
			...
			if ((0 != length)
					&& !attributes.elementAt(length - 1).isWhitespace())
	 ...
	 * @see com.bmtech.htmls.parser.Node#toHtml()
	 */
	@Override
	...
	/**
	 * Default tag visiting code. Based on <code>isEndTag()</code>, calls either
	 * <code>visitTag()</code> or <code>visitEndTag()</code>.
	 * 
	 * @param visitor
	...
	public boolean isEndTag() {
		...
		return ((null == raw) ? false : ((0 != raw.length()) && ('/' == raw
				.charAt(0))));
	}
	...
	/**
	 * Get the end tag for this (composite) tag. For a non-composite tag this
	...
	/**
	 * Set the end tag for this (composite) tag. For a non-composite tag this is
	 * a no-op.
	 * 
	 * @param end
	 *            The tag that terminates this composite tag, i.e.
	 *            &lt;/HTML&gt;.
	 */
	@Override
