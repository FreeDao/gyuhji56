...
//
// $Source: /liying/bmt_base/src/htmlps/com/bmtech/htmls/parser/tags/LinkTag.java,v $
// $Author: liying $
...
import com.bmtech.htmls.parser.util.ParserUtils;
...
public class LinkTag extends CompositeTag {
	 ...
	 */
	private static final String[] mEnders = new String[] { "A", "P", "DIV",
			"TD", "TR", "FORM", "LI" };

	/**
	 * The set of end tag names that indicate the end of this tag.
	 */
	private static final String[] mEndTagEnders = new String[] { "P", "DIV",
	...
	/**
	 * Constructor creates an LinkTag object, which basically stores the
	 ...
	 * 
	 * @return The value of the <code>ACCESSKEY</code> attribute, or
	...
	/**
	 * Returns the url as a string, to which this link points. This string has
	 * had the "mailto:" and "javascript:" protocol stripped off the front (if
	 * those predicates return <code>true</code>) but not for other protocols.
	 * Don't ask me why, it's a legacy thing.
	 * 
	...
	public String getLink() {
		if (null == mLink) {
			...
			if (javascript == 0) {
				mLink = mLink.substring(11); // this magic number is
												// "javascript:".length()
				javascriptLink = true;
			}
		}
	...
	public boolean isHTTPLink() {
		return (!isFTPLink() && !isHTTPSLink() && !isJavascriptLink()
				&& !isMailLink() && !isIRCLink());
	}
	...
	public String toString() {
		...
		sb.append("Link to : " + getLink() + "; titled : " + getLinkText()
				+ "; begins at : " + getStartPosition() + "; ends at : "
				+ getEndPosition() + ", AccessKey=");
	 ...
	 * 
	 * @param link
	 *            The new value of the <code>HREF</code> attribute.
	 */
	 ...
	 * 
	 * @return The URL from the HREF attibute. This is absolute if the tag has a
	 *         valid page.
	 */
