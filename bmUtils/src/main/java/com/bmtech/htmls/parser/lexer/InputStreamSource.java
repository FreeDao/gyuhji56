...
//
// $Source: /liying/bmt_base/src/htmlps/com/bmtech/htmls/parser/lexer/InputStreamSource.java,v $
// $Author: liying $
...
import com.bmtech.htmls.parser.util.EncodingChangeException;
...
public class InputStreamSource extends Source {
	 ...
	 * 
	 * @param in
	 *            Where to read this object from.
	 * @exception IOException
			...
			try {
				...
				try {
					...
					if (0 != offset) {
						...
						System.out.println(new_chars);
						for (int x = 0; x < offset; x++)
							System.out.print(buffer[x]);
						/***
						 * by mudie bug: if there is chanese charactor,the
						 * offset will wrong becouse a chanese charactor takes
						 * to position
						 */
						for (int i = 0; i < offset; i++)
							if (new_chars[i] != buffer[i])
								throw new EncodingChangeException(
										"character mismatch (new: "
												+ new_chars[i]
												+ " [0x"
												+ Integer.toString(
														new_chars[i], 16)
	...
	public long skip(long n) throws IOException, IllegalArgumentException {
		...
		if (0 > n)
			...
			if (mLevel - mOffset < n)
				fill((int) (n - (mLevel - mOffset))); // minimum to satisfy this
														// request
			if (mOffset >= mLevel)
				ret = EOF;
			else {
	 ...
	 * @exception IOException
	 *                If the offset is beyond {@link #offset()} or the source is
	 *                closed.
	 */
	@Override
	 ...
	 * @exception IOException
	 *                If the start or end is beyond {@link #offset()} or the
	 *                source is closed.
	 */
	@Override
