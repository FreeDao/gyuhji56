...
//
// $Source: /liying/bmt_base/src/htmlps/com/bmtech/htmls/parser/tags/CompositeTag.java,v $
// $Author: liying $
...
import com.bmtech.htmls.parser.nodes.AbstractNode;
import com.bmtech.htmls.parser.nodes.TagNode;
import com.bmtech.htmls.parser.scanners.CompositeTagScanner;
...
/**
 * The base class for tags that have an end tag. Provided extra accessors for
 * the children above and beyond what the basic {@link Tag} provides. Also
 * handles the conversion of it's children for the {@link #toHtml toHtml}
 * method.
 */
public class CompositeTag extends TagNode {
	...
	/**
	 * The tag that causes this tag to finish. May be a virtual tag generated by
	 * the scanning logic.
	 */
	...
	public Node getChild(int index) {
		return ((null == getChildren()) ? null : getChildren().elementAt(index));
	}
	...
	public Node[] getChildrenAsNodeArray() {
		return ((null == getChildren()) ? new Node[0] : getChildren()
				.toNodeArray());
	}
	 ...
	 *            The buffer to append to.
	 */
	protected void putEndTagInto(StringBuilder sb, boolean verbatim) {
		// eliminate virtual tags
		if (!verbatim
				|| !(mEndTag.getStartPosition() == mEndTag.getEndPosition()))
	 ...
	 *            If <code>true</code> return as close to the original page text
	 *            as possible.
	 * @return This tag and it's contents (children) and the end tag as HTML
	 *         code.
	 */
	@Override
	...
	/**
	 * Searches for all nodes whose text representation contains the search
	 * string. Collects all nodes containing the search string into a NodeList.
	 * This search is <b>case-insensitive</b> and the search string and the node
	 * text are converted to uppercase using an English locale. For example, if
	 * you wish to find any textareas in a form tag containing "hello world",
	 * the code would be: <code>
	 * NodeList nodeList = formTag.searchFor("Hello World");
	 * </code>
	 * 
	 * @param searchString
	 *            Search criterion.
	 * @return A collection of nodes whose string contents or representation
	 *         have the <code>searchString</code> in them.
	 */
	 ...
	 * "hello world", the code would be: <code>
	 * NodeList nodeList = formTag.searchFor("Hello World");
	 * </code>
	 * 
	 * @param searchString
	 *            Search criterion.
	 * @param caseSensitive
	 *            If <code>true</code> this search should be case sensitive.
	 *            Otherwise, the search string and the node text are converted
	...
	public NodeList searchFor(Class<?> classType, boolean recursive) {
		...
		if (null == children)
			...
			ret = children.extractAllNodesThatMatch(new NodeClassFilter(
					classType), recursive);
	...
	/**
	 * Returns the node number of the first node containing the given text. This
	 * can be useful to index into the composite tag and get other children.
	 ...
	 *         the text or -1 if not found.
	 * @param locale
	 *            The locale to use in converting to uppercase.
	 * @param text
	 *            The text to search for.
	 */
	public int findPositionOf(String text, Locale locale) {
		...
		for (SimpleNodeIterator e = children(); e.hasMoreNodes();) {
			node = e.nextNode();
			if (-1 != node.toPlainTextString().toUpperCase(locale)
					.indexOf(text))
				return loc;
			loc++;
		}
	...
	/**
	 * Collect this node and its child nodes (if-applicable) into the list
	 ...
	 * {@link CompositeTag}, and going through its children. So this method
	 ...
	 * 
	 * @param list
	 *            The list to add nodes to.
	 * @param filter
	 *            The filter to apply.
	 * @see com.bmtech.htmls.parser.filters
	 */
	@Override
	public void collectInto(NodeList list, NodeFilter filter) {
		...
		for (SimpleNodeIterator e = children(); e.hasMoreNodes();)
			e.nextNode().collectInto(list, filter);
		if ((null != getEndTag()) && (this != getEndTag())) // 2nd guard handles
															// <tag/>
	...
	/**
	 * Tag visiting code. Invokes <code>accept()</code> on the start tag and
	 * then walks the child list invoking <code>accept()</code> on each of the
	 * children, finishing up with an <code>accept()</code> call on the end tag.
	 * If <code>shouldRecurseSelf()</code> returns true it then asks the visitor
	 * to visit itself.
	 * 
	 * @param visitor
	 *            The <code>NodeVisitor</code> object to be signalled for each
	 *            child and possibly this tag.
	 */
	@Override
	public void accept(NodeVisitor visitor) {
		...
		if (visitor.shouldRecurseChildren()) {
			...
			if ((null != getEndTag()) && (this != getEndTag())) // 2nd guard
																// handles
																// <tag/>
	 ...
	 *  <LABEL>The label</LABLE>}, then this method would return the
	 * {@.html </LABLE>} end tag.
	 * 
	 ...
	 * 
	 * @param tag
	 *            The new end tag for this tag. Note: no checking is perfromed
	 *            so you can generate bad HTML by setting the end tag with a
	 *            name not equal to the name of the start tag, i.e.            {@.html
	...
	/**
	 * Finds a text node, however embedded it might be, and returns it. The text
	 * node will retain links to its parents, so further navigation is possible.
	 * 
	 * @param searchText
	 *            The text to search for.
	...
	/**
	 * Return a string representation of the contents of this tag, it's children
	 * and it's end tag suitable for debugging.
	 * 
