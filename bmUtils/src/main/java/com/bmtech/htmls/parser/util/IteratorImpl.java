...
//
// $Source: /liying/bmt_base/src/htmlps/com/bmtech/htmls/parser/util/IteratorImpl.java,v $
// $Author: liying $
...
public class IteratorImpl implements NodeIterator {
	...
	public boolean hasMoreNodes() throws ParserException {
		...
		ret = Page.EOF != mLexer.getPage().getCharacter(mCursor); // more
																	// characters?
		...
		try {
			...
			if (null != ret) {
				...
				if (ret instanceof Tag) {
					tag = (Tag) ret;
					if (!tag.isEndTag()) {
						// now recurse if there is a scanner for this type of
						// tag
