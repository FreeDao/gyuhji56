...
//
// $Source: /liying/bmt_base/src/htmlps/com/bmtech/htmls/parser/scanners/ScriptDecoder.java,v $
// $Author: liying $
...
/**
 * Decode script. Script obfuscated by the <A href=
...
public class ScriptDecoder {
	 ...
	 */
	protected static byte mEncodingIndex[] = { 1, 2, 0, 1, 2, 0, 2, 0, 0, 2, 0,
			2, 1, 0, 2, 0, 1, 0, 2, 0, 1, 1, 2, 0, 0, 2, 1, 0, 2, 0, 0, 2, 1,
			1, 0, 2, 0, 2, 0, 1, 0, 1, 1, 2, 0, 1, 0, 2, 1, 0, 2, 0, 1, 1, 2,
			0, 0, 1, 1, 2, 0, 1, 0, 2, };
	...
	public static String Decode(Page page, Cursor cursor)
		...
		while (STATE_DONE != state) {
			...
			if (Page.EOF == input_character) {
				if ((STATE_INITIAL != state) || (0 != substate_initial)
						|| (0 != substate_length) || (0 != substate_prefix)
						|| (0 != substate_checksum) || (0 != substate_final))
				...
				case STATE_LENGTH:
					...
					if (substate_length >= buffer.length) {
						...
						if (0 > length)
							throw new ParserException("illegal length: "
									+ length);
						substate_length = 0;
						state = STATE_PREFIX;
					}
				...
				case STATE_CHECKSUM:
					...
					if (substate_checksum >= buffer.length) {
						...
						if (check != checksum)
							throw new ParserException(
									"incorrect checksum, expected " + check
											+ ", calculated " + checksum);
						checksum = 0;
						substate_checksum = 0;
						state = STATE_FINAL;
	...
	// {
	// System.out.println
	// ("Usage: java org.htmlparser.scanners.ScriptDecoder <infile> <outfile> [charset]");
	// ret = 1;
	// }
	// else
	// {
