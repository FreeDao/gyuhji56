...
//
// $Source: /liying/bmt_base/src/htmlps/com/bmtech/htmls/parser/lexer/Source.java,v $
// $Author: liying $
 ...
 * <li>characters may be requested more than once, so in general they will be
 * buffered</li>
 * </ul>
 */
public abstract class Source extends Reader implements Serializable {
	...
	/**
	 * Set the encoding to the given character set. If the current encoding is
	 * the same as the requested encoding, this method is a no-op. Otherwise any
	 * subsequent characters read from this source will have been decoded using
	 * the given character set.
	 * <p>
	 * If characters have already been consumed from this source, it is expected
	 * that an exception will be thrown if the characters read so far would be
	 * different if the encoding being set was used from the start.
	 * 
	 * @param character_set
	 ...
	 *                exception is also thrown if the character set is not
	 *                recognized.
	 */
	...
	/**
	 * Does nothing. It's supposed to close the source, but use {@link #destroy}
	 * instead.
	 * 
	 ...
	 * available, an I/O error occurs, or the source is exhausted.
	 * 
	 * @return The character read, as an integer in the range 0 to 65535 (
	 *         <tt>0x00-0xffff</tt>), or {@link #EOF} if the source is
	 *         exhausted.
	 * @exception IOException
	 ...
	 * some input is available, an I/O error occurs, or the source is exhausted.
	 * 
	 * @param cbuf
	 *            Destination buffer
	 * @param off
	...
	/**
	 * Mark the present position. Subsequent calls to {@link #reset} will
	 * attempt to reposition the source to this point. Not all sources support
	 * the mark() operation.
	 * 
	 * @param readAheadLimit
	 *            The minimum number of characters that can be read before this
	 ...
	 * @exception IOException
	 *                If the source is closed or the start or end is beyond
	 *                {@link #offset()}.
	 */
	public abstract void getCharacters(char[] array, int offset, int start,
	...
	/**
	 * Append characters already read into a <code>StringBuilder</code>.
	 * 
	 * @param buffer
	 ...
	 *            The number of characters to retrieve.
	 * @exception IOException
	 *                If the source is closed or the offset or (offset + length)
	 *                is beyond {@link #offset()}.
	 */
	public abstract void getCharacters(StringBuilder buffer, int offset,
