...
public class AreaSeg {
	private static final AreaSeg instance = new AreaSeg();
	private String[] pattern = new String[] { "彝族.+", "哈尼族.+", "佤族.+", "土家族.+",
			"傣族.+", "拉祜族.+", "哈尼族.+", "苗族.+", "瑶族.+", "傈僳族.+", "苗族.+", "苗族",
			"畲族.+", "哈尼族", "保安族+", "东乡族.+", "哈萨克族.+", "蒙古族.+", "裕固族.+", "藏族.+",
			"回族.+", "白族.+", "独龙族.+", "彝族.+", "毛南族.+", "各族.+", "仡佬族.+", "侗族.+",
			"布依族", "拉祜族", "傣族", "拉祜族", "纳西族", "蒙古.+", "塔吉克.+", "哈萨克", "撒拉族.+",
			".翼.旗", "黎族.+", "仡佬族", "黎族", "满族.+", "黎族", "满族+", "壮族.+", "壮族 ",
			"土族.+", "水族.+", "壮族", "朝鲜族", "达斡尔族.+", "自治.+", "自治", "林区", "特区",
			"傈傈族.+" };
	...
	public static final int UNKOWN = 2703;
	...
	public class AreaSegment extends Segment {
		...
		public Set<String> getAllFullWord() {
			Set<String> ret = new HashSet<String>();
			Iterator<Entry<String, Short>> itr = map.entrySet().iterator();
			while (itr.hasNext()) {
				Entry<String, Short> e = itr.next();
				if (e.getValue() != state_may) {
					ret.add(e.getKey());
				}
			}
			return ret;
		...
		private AreaSegment() {
			...
			load("config/segment/area_lexicon", map, false, 15);
		}

		public void addItem(String item) {
			addItem(map, item, false, 15);
		}
	...
	private AreaSeg() {
		try {
			...
			while (lr.hasNext()) {
				...
				String sId = seg[0].trim();
				String rawName = seg[1].trim();
				String strParId = seg[2].trim();
				int id = Integer.parseInt(sId);
				int parId = Integer.parseInt(strParId);
				...
				Area a = new Area(id, name, parId);
				this.idMap.put(id, a);
			}
			lr.close();
			loadCapital();
			lr = new LineReader("./config/segment/fullName.txt",
					Charsets.UTF8_CS);
			while (lr.hasNext()) {
				String line = lr.next().trim();
				if (line.length() > 1) {
					segment.addItem(line);
				}
			}
			lr.close();
	...
	private void loadCapital() throws IOException {
		...
		for (Area a : col) {
			...
			if (t.parId == 0) {
				...
				if (a.name.length() > 2
						&& (a.name.endsWith("市") || a.name.endsWith("盟")
								|| a.name.endsWith("旗") || a.name
									.endsWith("特区"))) {
			...
			if (a.name.length() > 2
					&& (a.name.endsWith("县") || a.name.endsWith("市")
							|| a.name.endsWith("旗") || a.name.endsWith("特区"))) {
				if (a.name.endsWith("特区")) {
					String tmp = a.name.substring(0, a.name.length() - 2);
		...
		try {
			LineReader lr = new LineReader("config/areas/alias",
					Charsets.UTF8_CS);
			while (lr.hasNext()) {
				String line = lr.next();
				line = line.trim();
	...
	private void addCapital() throws IOException {
	...
	public List<Entry<Area, NumCount>> getAreas(final Area par, String title,
			String content) {
		...
		return guessAreas(grp.cntCountry.topEntry(5), grp.cntCity.topEntry(5),
				grp.cntProvince.topEntry(5));

	}

	private void getAreas(final Area par, CounterGroup grp, String input,
			final int baseScore) {
		if (input == null)
			return;
		...
		while (th.hasNext()) {
			...
			if (p != null) {
				...
				grp.cntProvince.count(p, score * baseScore * simScore(par, p));
			}
			...
			if (c != null) {
				if (p != null && (c.par == p)) {
					...
					grp.cntCity.count(c, score * baseScore * simScore(par, c));
				}
			}
	...
	public static String aliaName(Area a) {
		...
		if (a.name.length() > 2) {
			if (a.name.endsWith("县") || a.name.endsWith("市")
					|| a.name.endsWith("省")) {
	...
	public synchronized static List<Area> getChilds(int areaId) {
		if (childs.size() == 0) {
			Iterator<Area> itr = AreaSeg.instance.idMap.values().iterator();
			while (itr.hasNext()) {
				Area a = itr.next();
				if (a.parId > 0) {
			...
			itr = AreaSeg.instance.idMap.values().iterator();
			while (itr.hasNext()) {
				Area a = itr.next();
				if (a.parId != 0) {
					continue;
	...
	public static AreaSeg getInstance() {
		return instance;
	}
