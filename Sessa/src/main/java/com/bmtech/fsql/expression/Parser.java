...
import com.bmtech.fsql.expression.TokenType.TokenClass;
import com.bmtech.fsql.expression.ast.CompondCondtionNode;
import com.bmtech.fsql.expression.ast.ConditionNode;
import com.bmtech.fsql.expression.ast.ConjunctNode;
import com.bmtech.fsql.expression.ast.EndNode;
import com.bmtech.fsql.expression.ast.FieldNameNode;
import com.bmtech.fsql.expression.ast.FiledValueNode;
import com.bmtech.fsql.expression.ast.ItemConditionNode;
import com.bmtech.fsql.expression.ast.Node;
...
public class Parser {
	...
	private Token typeMatch(TokenClass tokenClass) {
		...
		if (token.type.tokenClass != tokenClass) {
			throw new ParserException("Expecting tokenClass " + tokenClass + " but got '" + token.text + "'",
					token.getPosition());
		}
		return token;
	}
	...
	private void matchLimit() {
		...
		if (next == null) {
			...
			if (lookAhead(1) != null) {
				throw new ParserException("expect no more token, but got " + lookAhead(1) + " at "
						+ lookAheadBuffer.getToken(0).getPosition());
			}
		} else {
			throw new ParserException(
					"expect asc/desc/limit but got " + next + " at " + lookAheadBuffer.getToken(0).getPosition());
		}
	}

	private ConditionNode statements(ConjunctNode conjunctNode) {
		...
		if (type.tokenClass == TokenClass.CONJUNCT) {
			...
			if (lookAhead(2) == TokenType.LPAREN) {
				...
				FieldNameNode fieldName = new FieldNameNode(variable.getPosition(), variable.text);
				...
				if (valueToken.type == TokenType.STRING_LITERAL) {
					...
					throw new ParserException(
							"expect value or '?', but get " + valueToken + " @" + valueToken.getPosition());
				}
				ItemConditionNode condNodeX = new ItemConditionNode(valueToken.getPosition(), fieldName, relation,
						valueNode);
				if (isVar) {
	...
	private Node checkConjunct() {
		...
		if (t.type.tokenClass == TokenClass.CONJUNCT) {
			...
			throw new ParserException("expect Connjunct or ')' but get " + this.lookAheadBuffer.getToken(0) + " at "
					+ lookAheadBuffer.getToken(0).getPosition());
		}
	}
