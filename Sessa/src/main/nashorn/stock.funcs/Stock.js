...
var Stock = function(stkInfo){
	...
	if(!yearData){
	    var file = new File(stockDayDir , this.getCode() +"/" + year);
	    if(!file.exists()){
		yearData = loadVar(file);//FIXME should inject StockDay's functions
	...
	if(intDay <= nowDay('yyyy') && intDay >= 1994){
	    intDay = intDay * 10000 + 0101
	}else if(intDay <= nowDay('yyyyMM') && intDay >= 199401){
	    intDay = intDay * 100 + 01
	}else{
	...
	var dayTo = dayTo ? dayTo : nowDay("yyyyMMdd");
	var intDayTo = this.dayIntFormat(dayTo);
     ...
     * @param from {yyyyMMdd} 开始时间
     * @param from {yyyyMMdd} 结束时间
